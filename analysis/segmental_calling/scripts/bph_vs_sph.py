...
if __name__ == "__main__":
    ...
    centromere_df = pd.read_csv(
        snakemake.input["centromere_bed"], header=None, sep="\s+"
    )
    centromere_df.columns = ["chrom", "start", "end", "feature"]
    bp_padding = snakemake.params["bp_padding"]
    min_centromere_pos = np.nanmin(
        centromere_df[centromere_df.chrom == snakemake.wildcards["chrom"]].start.values
    )
    max_centromere_pos = np.nanmax(
        centromere_df[centromere_df.chrom == snakemake.wildcards["chrom"]].end.values
    )
    ...
    (
        bfs_bph_sph_centromere,
        posterior_bph_centromere,
        posterior_sph_centromere,
    ) = bph_vs_sph_trisomy(
        gammas,
        states,
        pos=pos,
        start=min_centromere_pos - bp_padding,
        end=min_centromere_pos + bp_padding,
    )
    # Step 2. Get the centromeric regions associated appropriately here
    (
        bfs_bph_sph_noncentro,
        posterior_bph_noncentro,
        posterior_sph_noncentro,
    ) = bph_vs_sph_trisomy(
        gammas,
        states,
        pos=pos,
