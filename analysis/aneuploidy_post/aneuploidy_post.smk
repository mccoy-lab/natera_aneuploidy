...
rule isolate_trisomies:
    ...
    run:
        ...
        trisomy_df = aneuploidy_df[(aneuploidy_df.bf_max_cat == "3m") | (aneuploidy_df.bf_max_cat == "3p")][["mother", "father", "child", "chrom"]]
...
def expand_bph_sph(fp="results/bph_sph/valid_trisomies.tsv"):
    ...
    res_files = [] 
    for (m,p,c,chrom) in zip(trisomy_df.mother.values, trisomy_df.father.values, trisomy_df.child.values, trisomy_df.chrom.values):
