...
alpha = 10 #the lower percentile cutoff
assert alpha > 0
assert alpha < 100
...
for i,p in enumerate(ploids):
    ...
    thresholds[p] = x
    ax[i].hist(data, density=False, bins=50)
    if alpha == 1:
        ax[i].axvline(x, color='orange', linestyle='--', label=f'{int(alpha)}st percentile: {x:.2f}')
    else:
        ax[i].axvline(x, color='orange', linestyle='--', label=f'{int(alpha)}th percentile: {x:.2f}')
    ax[i].set_title(p)
...
plt.savefig('pdfs/figS42.pdf', bbox_inches='tight')
